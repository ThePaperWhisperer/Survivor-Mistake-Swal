...
clearTimeout(timeout)
	if(night == false){
	 ...
	 day.hidden = false;
		...
		if(daynumber === 6){
			day.innerHTML = "Day " + daynumber + ". look for an abandoned building near (1000, 1000).";
			model.style.scale = "3 3 3";
			model.style.position = "absolute";
			model.style.height = "1000px";
			model.style.width = "1000px";
			model.style.transform = "translate3d(-1000px, 0px, 1000px) perspective(2000px)";
			task.max = 1;
													 task.value = 1;
...
function load() {
	...
	setTimeout(() => {
		...
		if(document.activeElement != document.getElementById("message")){
			...
			if(e.key == "Enter" && daynumber > 5 && matrix4.m41 === -1000 && matrix4.m43 === 1000){
